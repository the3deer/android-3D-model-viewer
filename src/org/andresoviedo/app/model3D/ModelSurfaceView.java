...
class ModelSurfaceView extends GLSurfaceView {
	...
	public synchronized boolean onTouchEvent(MotionEvent motionEvent) {
		...
		if (pointerCount == 1) {
			...
			isRotating = !isOneFixedAndOneMoving && (dx1 != 0 && dy1 != 0 && dx2 != 0 && dy2 != 0) && rotationVector[2] != 0;
		}

		if (touchDelay > 1) {
			// INFO: Procesar gesto
			if (pointerCount == 1 && currentPress1 > 4.0f) {
				hitSquare = hit(hit1, hit2, mRenderer.getmSquare().getPosition());
				hitTriangle = hit(hit1, hit2, mRenderer.getmTriangle().getPosition());
			} else if (pointerCount == 1) {
				touchStatus = TOUCH_STATUS_MOVING_WORLD;
				Log.i("Camera", "Moving World '" + dx1 + "','" + dy1 + "'...");
				mRenderer.getCamera().translateCamera(dx1 * 10 / mRenderer.getWidth(), dy1 * 10 / mRenderer.getHeight());
			} else if (pointerCount == 2) {
				if (fingersAreClosing) {
					touchStatus = TOUCH_STATUS_ZOOMING_CAMERA;
					float zoomFactor = (length - previousLength) / ((mRenderer.getWidth() + mRenderer.getHeight()) / 2) * 10;
			...
			// mRenderer.getmSquare().setRotationZ(mRenderer.getmSquare().getRotationZ() + ((dx + dy) * TOUCH_SCALE_FACTOR));
			// } else if (wzTriangle > wzSquare) {
			// mRenderer.getmTriangle().setRotationZ(mRenderer.getmTriangle().getRotationZ() + ((dx + dy) * TOUCH_SCALE_FACTOR));
			// }
			//
			// break;
			//
			...
			//
			// if (sqHit) {
			// mRenderer.getmSquare().translateX((dx * mRenderer.getRatio() * 2 / mRenderer.getWidth()) * TOUCH_MOVE_FACTOR);
			// mRenderer.getmSquare().translateY((-dy * 1 / mRenderer.getHeight()) * TOUCH_MOVE_FACTOR);
			// }
			// if (triHit) {
			// mRenderer.getmTriangle().translateX((dx * mRenderer.getRatio() * 2 / mRenderer.getWidth()) * TOUCH_MOVE_FACTOR);
			...
			// // Rotate Camera
			// // TODO: Rotationfactor debería ser proporcional a la z?
			// if (wzSquare > wzTriangle) {
			// mRenderer.getmSquare().setRotationZ(mRenderer.getmSquare().getRotationZ() + (actualRotation * TOUCH_ROTATION_FACTOR));
			// } else if (wzTriangle > wzSquare) {
	...
	public float[] unproject(float rx, float ry, float rz) {
		...
		GLU.gluUnProject(rx, ry, rz, mRenderer.getmViewMatrix(), 0, mRenderer.getmProjectionMatrix(), 0, viewport, 0, xyzw, 0);
